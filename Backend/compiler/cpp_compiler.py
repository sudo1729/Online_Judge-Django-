...
def executeCppCode(data):
    ...
    try:
        s = subprocess.check_output("g++ code.cpp -o out",stderr=subprocess.PIPE, shell = True)
        try:
            t = subprocess.check_output("./out",stderr=subprocess.PIPE, stdin = dataX,shell = True)
